...
pub fn set_invocation_time<P: AsRef<Path>>(path: P) -> Result<FileTime> {
...
pub fn create_dir_all<P: AsRef<Path>>(p: P) -> Result<()> {
...
fn set_not_readonly<P: AsRef<Path>>(p: P) -> io::Result<bool> {
    let p = p.as_ref();
